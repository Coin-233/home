<template>
    <div class="progress-container">
      ...
      <div class="day-progress-text">
        今日已过 {{ dayProgress.toFixed(4) }}%
      </div>
      <div class="year-progress-bar">
        <div class="year-progress" :style="{ width: `${progress}%` }"></div>
      </div>
      <div class="year-progress-text">
        今年已过 {{ progress.toFixed(4) }}%
      </div>
    </div>
  </template>
  ...
  export default defineComponent({
    setup() {
      ...
      const updateYearProgress = () => {
        ...
        progress.value = ((now.getTime() - start.getTime()) / (end.getTime() - start.getTime())) * 100;
      };
      ...
      const updateDayProgress = () => {
        ...
        dayProgress.value = ((now.getTime() - start.getTime()) / (end.getTime() - start.getTime())) * 100;
      };
  ...
  .progress-container {
    ...
    border-radius: 5px;
    margin-top: 20px;
    padding: 20px; /* 增加内边距以容纳两个进度条 */
  }
  
  .day-progress-bar, .year-progress-bar {
    width: 100%;
    height: 10px;
  ...
  .day-progress, .year-progress {
    ...
    height: 100%;
    transition: width 0.5s; /* 进度条宽度过渡效果 */
  }
  
  .day-progress-text, .year-progress-text {
    text-align: center;
    color: #000;
    font-size: 14px;
    font-weight: bold;
